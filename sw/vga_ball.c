...
#define WIDTH(x) ((x) + 3)
#define HEIGHT(x) ((x) + 4)
#define STROBE_OFF(x) (x)
#define STROBE_ON(x) ((x) + 1)
...
static char read_key(bool strobe)
{
	if (strobe)
	{
...
{
	iowrite8(x, WIDTH(dev.virtbase));
	iowrite8(y, HEIGHT(dev.virtbase));
	dev.ball_x = x;
	dev.ball_y = y;
}
